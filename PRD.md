...
**作成者**: Ryusei | **日付**: 2026-03-29 | **ステータス**: MVP実装済み（Phase 2計画中）
**アイデアID**: 1 | **ドメイン**: Security / AI Agent | **難易度**: High

> **現在の実装境界**: MVPはMCPプロキシ、OAuth2トークン検証、監査ログまでを提供する。OPAによるツール粒度の認可は未実装であり、認証済みユーザーは全upstream toolへアクセスできる。OPA統合はPhase 2で実装する（[ADR-003](docs/adr/0003-mvp-opa-deferred.md)）。ReBAC/OpenFGAは、リソース粒度の認可が必要になった場合に別ADRで再検討する。
...
| ツール粒度の認可ポリシー適用（Phase 2） | ポリシー評価のレイテンシ | < 10ms (p99) |
...
- **OPA (Open Policy Agent)** — Phase 2で統合するポリシーエンジン（ツール粒度認可）
...
- **Go プロキシ**: MCP over HTTP/SSE のリクエストを受信し、MVPではトークン検証→上流転送→監査ログの一連の処理を実行。Phase 2でポリシー評価を追加する
...
**差別化ポイント**: MCP 2026ロードマップが明示的に「ゲートウェイパターン未定義」「認可伝播の標準なし」と認めている。MVPではMCPプロキシ、OAuth2認証、監査ログを一体提供し、Phase 2でOPAによるツール呼び出し粒度の制御を追加する。ReBACは現行機能ではなく、必要性が確認された場合の将来検討とする
...
| OPAポリシー設計の学習コスト | 低 | 中 | Phase 2では単純なallow/denyポリシーから開始。Regoの複雑なパターンは後続フェーズで検討 |
...
- OPAはADR-002で採用済み、MVPでの統合延期はADR-003で決定済み。OpenFGA/ReBACはリソース粒度の認可が必要になった場合に別ADRで再検討
